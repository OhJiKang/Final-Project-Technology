$(document).ready(() => {
  ...
  function clickAvailableBadge() {
    ...
    $(this)
      .removeClass("bg-primary available_choosing")
      .addClass("bg-success available_cancel");
  }
  function removeClikedBadge() {
    ...
    $(this)
      .removeClass("bg-success available_cancel")
      .addClass("bg-primary available_choosing");
  }
      ...
      success: function (data) {
        if (!data.error) {
          ...
          data.consequent.forEach((element, index) => {
            htmls += `<span class="badge bg-primary" id=${index} style="margin-right:8px">${element}</span>`;
          });
